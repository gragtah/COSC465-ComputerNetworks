//LAB 06
//BY GAURAV RAGTAH, CAROLINE CRAGIN, AJ GANIK
...
void run(int sock, sockaddr_in sin, char* mode)
{
	...
	else //run as client
	{
                ...
                if (inet_pton(AF_INET, host, &sin.sin_addr) <= 0) 
                {
                      ...
                      int error = getaddrinfo(host, 0, &hints , &result);
                      if (error == 0) {
                          struct sockaddr_in *temp = (struct sockaddr_in*)(result ->ai_addr);
                          memcpy(&sin.sin_addr , &temp->sin_addr , sizeof(sin.sin_addr));
                          freeaddrinfo(result);
                      } 
                      else{
                          std::cerr << "Couldn't resolve IP/hostname" << host << std::endl;
                          close(sock);
                          exit(0);
                      }
                }
        ...
        int valid = 1, timeout = 10000, fileend = 0; 
        while (valid) 
        {
                    struct pollfd pfd[2];
                    ...
                    int rv = poll(pfd, 2, timeout); //timeout is 10000 by default, as declared above.
                    if (rv == 0)
                    {
                      if(timeout > 1500) { std::cerr << "Poll timed out. Goodbye." << std::endl;}
                    ...
                    {
                            if (pfd[0].revents) //data coming from network
                            {
                                    char buffer[4096];
                                    ...
                                    {
                                        std::cerr << "Didn't get anything from the other side.  Weird." << std::endl;
                                    }
                            }
                            if (pfd[1].revents) //data coming from stdin
                            {
                                    ...
                                    if(!std::cin.eof())
                                    {
                                        ...
                                        if (std::cin.eof()) 
                                        {
                                           if (!flag) {close(aSock); exit(0);} //Server should exit, but client will wait for first timeout
                                    ...
                                    {
                                       timeout=800;
                                        if (fileend==1)  //first timeout already. exit!
                                        {
                    ...
                    else {	std::cerr << "Didn't get anything from the other side.  Weird." << std::endl;}
            } //main while-loop end
}
