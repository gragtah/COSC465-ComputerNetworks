...
int main(int argc, char **argv)
{
    ...
    double factor = 1;
    //set experimentally
    if (target_rate > 10000000) { factor = 0.56;}
    for(i=0; i<num_packets; i++){
        ...
        if (i < num_packets - 1  ){ 
         wait_timex = (double) factor*((total_time*1000) - send_time.tv_usec + (start_time.tv_sec - send_time.tv_sec)*1000000 + start_time.tv_usec)/ (double) (num_packets -1-i ); 
