...
//    std::cout << "bitblast trying to send packet"<<(i+1) << std::endl;
        ...
        if (i < num_packets - 1) {
       // if (i % 2==0) {
         wait_timex = (double) 0.46* ((total_time*1000) - send_time.tv_usec + (start_time.tv_sec - send_time.tv_sec)*1000000 + start_time.tv_usec)/ (double) (num_packets -1-i );//;CHANGE HERE 
