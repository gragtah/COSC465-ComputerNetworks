...
int main(int argc, char **argv)
{
    ...
    std::string recvrip = "";
    int num_packets = 0;
    double  target_rate = 0; //previously int
    int packet_size = 0;
    ...
    int groupsize=num_packets/5; //  /10? 20? Also handle case where becomes 0, set as 1.
   ...
   // double wait_time = (double) total_time/ (double) (num_packets -1 ); 
    ...
    double factor = 1;
    //set experimentally
    if (target_rate > 10000000) { factor = 0.5;}
    for(i=0; i<num_packets; i++){
          
        //for(j=0; j<groupsize && i<num_packets; j++){
        //command
        if (i< num_packets-2) { 
       ...
       // i++;
       // }
        if (i < num_packets - 1  ){ //&& i%groupsize ==0) {
       // if (i % 2==0) {
         wait_timex = (double) factor*((total_time*1000) - send_time.tv_usec + (start_time.tv_sec - send_time.tv_sec)*1000000 + start_time.tv_usec)/ (double) (num_packets -1-i );//;CHANGE HERE 
